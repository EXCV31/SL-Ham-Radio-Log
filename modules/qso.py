...
from helpers.current_date import get_today
...
def register_qso():
    ...
    # Loop is here for continuously adding a QSO. User can exit if put empty callsign.
    cleared = False
    while True:
        if cleared is False:
            clear_console()
        ...
        # Insert user QSO to database.
        cursor.execute("INSERT INTO QSO (DATA, ZNAK, PASMO, MODULACJA) VALUES (?,?,?,?)",
                       (today, qso_callsign, band, modulation))
        logging.info("Umieszczono QSO w bazie danych.")
        conn.commit()
